...
with open('course.json') as f:
    ...
    for course in courses:
        ...
        dept.append(course["field_name_en"])
        program.append((course["program_name_en"]))
        language_type.append((course["program_type_name_th"]))

uni = pd.DataFrame([university_id, university_name, campus, faculty, dept, program, language_type]).transpose()
uni.columns = ['UNI_ID', 'UNI_NAME', 'CAMPUS', 'FACULTY', 'DEPT', 'PROGRAM', 'LANGUAGE_TYPE']
uni.dropna(inplace=True)
uni.to_csv(r'/Users/thanapatpiyamssakul/coding/first_year/de141/courses.csv', index=False)
